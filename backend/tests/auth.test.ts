...
import { verifyAccessToken, verifyRefreshToken } from '../src/utils/jwt.util';
...
const runTests = async () => {
  ...
  await test('Valid user registration creates user and returns safe payload without passwordHash', async () => {
    ...
    assert.ok(result.user, 'Result must contain user object');
    assert.strictEqual(result.user.email, testEmail);
    assert.strictEqual(result.user.role, UserRole.STUDENT);
    assert.strictEqual((result.user as any).passwordHash, undefined, 'passwordHash must never be exposed');
    assert.ok(result.user.id, 'User must have an id');
    assert.ok(result.user.uuid, 'User must have a uuid');
  ...
  await test('Valid login returns tokens, user payload and updates lastLoginAt', async () => {
    ...
    assert.ok(dbUser!.lastLoginAt! >= beforeLogin, 'lastLoginAt must be recent');
  ...
  await test('resetPassword rejects invalid or re-used reset token', async () => {
    ...
    try {
      await authService.resetPassword({
        token: resetTokenStr, // already used or forged
